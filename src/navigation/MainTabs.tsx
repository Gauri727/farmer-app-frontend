...
/* =========================================================
   CENTER VOICE ASSISTANT FAB
========================================================= */

const FloatingCenterFab: React.FC<{
  onPress: () => void;
  isFocused?: boolean;
}> = ({ onPress, isFocused }) => {
...
const TabItem: React.FC<{
  ...
  const scaleValue = React.useRef(
    new Animated.Value(isFocused ? 1.08 : 1)
  ).current;
...
const CustomTabBar: React.FC<BottomTabBarProps> = ({
  ...
  const currentChildRoute =
    (
      state.routes[state.index]?.state as any
    )?.routes?.[
      (
        state.routes[state.index]?.state as any
      )?.index || 0
  ...
  const isVoiceAssistantFocused =
    currentRouteName === 'HomeTab' &&
    currentChildRoute === 'VoiceAssistant';
  ...
  const tabs: TabConfig[] = [
    ...
    {
      routeName: 'EligibilityTab',
      label: t('eligibilityTab'),
      ...
      {tabs.map((tab) => {
        ...
        const routeIndex =
          state.routes.findIndex(
            (r) =>
              r.name === regularTab.routeName
          );

        const isFocused =
          state.index === routeIndex &&
          !isVoiceAssistantFocused;
        ...
        return (
          <TabItem
            ...
            activeIcon={
              regularTab.activeIcon
            }
            inactiveIcon={
              regularTab.inactiveIcon
            }
            onPress={onPress}
          />
        );
