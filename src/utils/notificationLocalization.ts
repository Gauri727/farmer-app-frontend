 ...
 * Localizes notification titles, bodies, categories, and time ago
...
export const NOTIFICATION_TRANSLATIONS: Record<string, LocalizedNotificationData> = {
  n1: {
    title: {
      en: 'Application window open for Drip Irrigation Scheme',
    ...
    body: {
      en: 'State Agriculture Department is accepting applications for micro-irrigation subsidies.',
  ...
  n2: {
    title: {
      en: 'Last date extended for Soil Health Card testing',
    ...
    body: {
      en: 'Farmers can get free soil testing done at nearest Krishi Vigyan Kendra till next month.',
...
const TIME_AGO_STRINGS: Record<string, { today: string; yesterday: string; daysAgo: string; monthsAgo: string }> = {
  mr: { today: 'आज', yesterday: 'काल', daysAgo: '{count} दिवसांपूर्वी', monthsAgo: '{count} महिन्यांपूर्वी' },
  en: { today: 'Today', yesterday: 'Yesterday', daysAgo: '{count} days ago', monthsAgo: '{count} months ago' },
  hi: { today: 'आज', yesterday: 'कल', daysAgo: '{count} दिन पहले', monthsAgo: '{count} महीने पहले' },
  ahr: { today: 'आज', yesterday: 'काल', daysAgo: '{count} दिवसांपूर्वी', monthsAgo: '{count} महिन्यांपूर्वी' },
  kok: { today: 'आज', yesterday: 'काल', daysAgo: '{count} दिसां आदीं', monthsAgo: '{count} म्हयन्यां आदीं' },
};
...
export const getLocalizedNotification = (
  ...
  let trans = NOTIFICATION_TRANSLATIONS[notification.id];

  if (!trans && notification.title) {
    const titleLower = notification.title.toLowerCase();
    if (titleLower.includes('drip') || titleLower.includes('irrigation')) {
      trans = NOTIFICATION_TRANSLATIONS['n1'];
    } else if (titleLower.includes('soil') || titleLower.includes('kvk')) {
      trans = NOTIFICATION_TRANSLATIONS['n2'];
    } else if (titleLower.includes('pm-kisan') || titleLower.includes('kisan')) {
      trans = NOTIFICATION_TRANSLATIONS['n3'];
    } else if (titleLower.includes('fasal') || titleLower.includes('bima') || titleLower.includes('insurance')) {
      trans = NOTIFICATION_TRANSLATIONS['n4'];
    }
  }
...
export const getLocalizedTimeAgo = (dateString: string, langCode: string): string => {
  ...
  if (!dateString) {
    return dict.daysAgo.replace('{count}', '2');
  }
  ...
  return dict.monthsAgo.replace('{count}', String(diffMonths));
};
